...
def real_frequency_generation(dataset, data_size, domain_size, data_dimension, query_interval_table):
    ...
    if data_dimension==1:
        ...
        for k,item in enumerate(dataset):   #tuple类型
    ...
    for i, query_interval in enumerate(query_interval_table):
        ...
        for j in range(data_dimension):
            user_histogram_copy = np.swapaxes(user_histogram_copy, 0, j)  #交换数组两个轴，用在多维上[1,2,3]
            user_histogram_copy = user_histogram_copy[query_interval[2 * j]: query_interval[2 * j + 1] + 1]
