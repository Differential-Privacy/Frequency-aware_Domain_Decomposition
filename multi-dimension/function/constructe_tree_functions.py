...
def leaf_node_average_level_order(tree,eps,tree_max_height,data_size):
    ...
    def find_min_max_value(average_leaf_node):
        min_value = min(min(node.data.interval) for node in average_leaf_node)
        max_value = max(max(node.data.interval) for node in average_leaf_node)
        ...
        for child in children:
            if not child.data.divide_flag:
                if child.data.frequency<=mju*error_oue:
                    ...
                    leaf_avarage_list.append(child.data.interval)
                    current_begin,current_end = find_min_max_value(average_leaf_node)
                    leaf_avarage_list.append([current_begin,current_end])
    ...
    sorted_avarage_leaf = sorted(leaf_avarage_list, key=lambda x: x[0])


    return sorted_avarage_leaf




def level_order(tree,eps,tree_max_height,data_size,user_alpha,d,C_d_2,g):
    e_eps=np.exp(eps)
...
def level_order_2(tree,eps,tree_max_height,data_size,user_alpha,d,C_d_2,initial_interval_list):
    e_eps=np.exp(eps)
    error_c=np.sqrt(np.sqrt(4*C_d_2*e_eps*user_alpha/(data_size*d*(1-user_alpha)*(e_eps-1)**2)))
    ...
    while queue:
        ...
        non_leaf_node = []
        for child in children:
            if child.data.frequency<error_c*c_mju: 
                tmp_interval_fre_list.append([child.data.interval,child.data.frequency])
            else:
                if not child.data.divide_flag:
                    tmp_interval_fre_list.append([child.data.interval, child.data.frequency])
    ...
    return res_intervals if res_intervals else [[root.data.interval[0],root.data.interval[1]]]
    ...
    for node in tree.leaves():
        ...
        if not node.data.divide_flag: 
            ...
            if tree_height == 0:  
                ...
                mi = math.ceil((n * f ** 2 * (e_eps - 1) ** 2 * alpha ** 2 / e_eps) ** (1 / 3))
                error_oue = math.sqrt(4 * e_eps * h_yuce / (e_eps - 1) ** 2 / (data_size - node.data.user_end))  
...
def non_negative(Domain, tree):
    for level in range(1, tree.depth() + 1):
        ...
        while True:
            ...
            if flag == 0:
                break
            values = sum(non_negative_frequency)  
...
def consistency_adjustment(tree):
    ...
    for subtree_root in tree.children('root'):
        ...
        for layer_order_node in subtree_layer_order:
            ...
            if subtree_node.data.divide_flag:  #非叶节点
...
def collect_leaves(tree):
    ...
    leaf_list.sort(key=lambda t: t.data.interval[0])   # 排序
